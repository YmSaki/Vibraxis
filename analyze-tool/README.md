...
`rigidGrid: true`（要 `bpm` 併記）は拍グリッドを **grid solver 方式**へ切り替えます:
動的追跡（librosa beat_track）の出力を捨て、固定 BPM の等間隔グリッドを生成し、
アンカー位相だけを onset 強度との整合が最大になる位置に全探索で合わせます。
DAW 制作の一定テンポ素材では動的追跡より正確です（追跡は onset の弱い区間で
拍間隔が揺れる）。ダウンビート・小節・コード・構成・pad は全てこのグリッドから
再導出されます。可変テンポの生演奏素材には使わないでください。
...
- ダウンビートは4/4前提で、位相を**拍同期のクロマ変化（和声の変わり目＝コードは"1"で変わる）**から
  推定します。サンプル16曲での位相一致は旧onset方式の5/16(31%)に対し11/16(69%)。残りは±半小節の
  曖昧性で`downbeatOffsetBeats`による人手補正が必要（12→5曲に減少）。変拍子は別providerが必要です。
- 構成ラベルは**音響的な形状**（曲内相対エネルギー、低域=キック有無、区間内のエネルギー傾き=build判定、
  和声反復、build→drop隣接）から導く低信頼ヒューリスティックです。境界は低域のキックon/off遷移と
  音色/和声noveltyから採ります。人手正解注釈が未整備のため状態は`partial`のままです。
