...
- Free for personal and commercial use; no permission inquiry required.
- Credit is not required (but appreciated — hence this file).
- Arrangement and modification are allowed (this project applies tempo adjustment and crossfading at playback time).
- Redistribution is not prohibited, provided the tracks are not passed off as one's own work and not sold as-is.
  This repository redistributes the unmodified audio files solely as sample data for the Vibraxis demo,
  with full attribution to BGMer.
- BGMer retains the master and composition copyrights. **The tracks are NOT covered by this repository's
  code license** — they remain under BGMer's terms.
- BGMer does not register these tracks with YouTube Content ID, so demo videos using them are safe.

Prohibited by BGMer (and not done by this project): registering the tracks with copyright-management
organizations or audio fingerprint systems (e.g. YouTube Content ID), redistributing under a false author,
selling the tracks as-is.
...
See `analyze-tool/README.md` for audio-analysis dependencies (librosa, ISC License).
